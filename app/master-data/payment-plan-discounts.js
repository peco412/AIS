...
const PLAN_LABEL = { sublevel: 'Theo cấp độ con', level: 'Theo cấp độ', program: 'Theo chương trình' };
let PROFILE = null;
let CAN_EDIT = false;

async function loadRows() {
  const tbody = document.getElementById('tableBody');
  const { data, error } = await supabase.from('payment_plan_discounts').select('*, employees(full_name)').order('plan_type');
  ...
  tbody.querySelectorAll('[data-save]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      const { error: err } = await supabase.from('payment_plan_discounts')
...
(async () => {
  try {
    ...
    CAN_EDIT = PROFILE.departmentCode === 'ACC' || profile.roleCode === 'TECH';
    await loadRows();
